<script lang="ts">
  ...
  interface Props {
    pythonReadyPromise: Promise<any>;
    index: number;
  ...
  let generatedCode = $derived.by(() => {
    if (statement && result && "generatedCode" in result && result.generatedCode) {
      try {
        ...
        return `# Error generated code: ${e} If this error persists, report to https://github.com/animagr/EngineeringPapyr/issues with the sheet that generates the error.`
      }
    } else {
      return "";
    }
  ...
  function codeTemplate(statement: CodeFunctionQueryStatement, result: Result | FiniteImagResult | MatrixResult) {
    ...
    """
    Function '${statement.functionName}' automatically generated by EngineeringPapyr
...
<div class="info">
  ...
  <div>
    ...
    <a href="mailto:https://github.com/animagr/EngineeringPapyr/issues">https://github.com/animagr/EngineeringPapyr/issues</a>
  </div>
</div>
